...
- **the CRM CRM** (all reads and writes: deals, contacts, orgs, activities, notes)
...
- **the platform** (all reads via Python SDK)
...
| memory/reference/platform-sdk.md | Any platform data pull (methods, fields, rules) |
...
# the platform Access
**The the platform MCP is NOT available in Cowork sessions.** Use the Python SDK directly via bash.
```python
from platform_sdk import PublicClient, Client
...
```
> Full SDK methods + fields: memory/reference/platform-sdk.md
