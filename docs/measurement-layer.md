...
- `social-post.md` — Twitter/X + LinkedIn
- `client-linkedin.md` — client LinkedIn posts
- `email.md` — newsletters + product updates
...
```json
{
  ...
  "context": { "profile": "house", "archetype": "product_data" }
}
```
...
**Across skills.** Same infra works for audits (`client-onboarding`), briefs (`competitive-intel-scanner`), emails (`email-newsletter-drafter`), scheduled task outputs, even `branded-doc-generator` deliverables. One shared measurement layer, dozens of beneficiaries.
...
(Paul Iusztin, "Designing Real-World AI Agents"). JakeOS stores judge logs as local JSONL instead of Opik, and the rubrics are tuned to house brand voice rather than generic LinkedIn style, but the core pattern is theirs. Full credit.
...
- `memory/datasets/house-social/seeds.md` — 10 seeds
- `memory/datasets/client-linkedin/seeds.md` — 5 house-level seeds

Per-client seed sets live with the client profiles under `.claude/skills/client-linkedin-voice/clients/`.

## Profile loaders

Alongside the measurement layer, two new consolidated profile files were added:
- `memory/reference/profiles/house.md`
- `memory/reference/profiles/house-profile.md`
...
Client LI: avg 7.8 (flat). Weakest: closing_question 6.9.
...
|---|---|---|
| Client audits | `audit.md` — scorecard clarity, roadmap specificity, competitive differentiation, ICP fit | Every `client-onboarding` run before PDF delivery |
...
- [x] client rubric: voice_match capped at 5.0 when no client profile loaded
...
- [x] Pipeline coverage notes in `memory/datasets/house-social/README.md`
...
- [ ] Apply PATCH 2 to `client-linkedin-voice/SKILL.md` (same file)
...
- [ ] Populate `memory/datasets/house-social/exemplars.md` with top 5 real posts (quarterly recalibration)
