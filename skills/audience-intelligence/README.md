...
| Event | Score | Why |
|-------|-------|-----|
| `signup.start` | +5 | Raised hand |
| `signup.success` | +5 | Converted |
| `product.compare-models` | +3 | Active product evaluation |
| `product.home-cta-click` | +2 | Conversion intent |
| `auth.sign-in.start` | +2 | Returning user signal |
| `auth.sign-in.success` | +2 | Active user |
| `product.share-result.start` | +1 | Amplification intent |
...
3. **Sign-ups** (complete contact data, push to Clay for enrichment then the CRM)
...
10. **Top Countries**
11. **Action Items** (specific list of who to push to Clay/the CRM this week)
...
Combined score = Clay Tier Score + Mixpanel Behavior Score
    |
    v
Score >= 13: Push to the CRM (CRM visibility)
...
- `customer@example.com` (an existing customer, existing customer, behavior score 7)
