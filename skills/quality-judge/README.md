...
3. **context** — optional. Profile reference, source material, brief the draft was written from, prior iteration score. For the client drafts, MUST include `client` slug or voice_match is capped.
...
```json
{
  ...
  "context": {"profile":"house","archetype":"product_data","platform":"twitter"}
}
```
...
- `client-linkedin.md` — client LinkedIn
- `email.md` — emails
...
python3 scripts/quality_log.py --rubric client-linkedin --since 30d --bias-check
```
...
- Scoring without context when context is required (client drafts always need a client slug).
- Grade inflation.
