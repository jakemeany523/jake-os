...
description: |
  ...
  Trigger on: "run the loop", "optimize this draft", "iterate until it passes", "evaluator-optimizer", or any call from another skill (social-content-drafter, client-linkedin-voice, email-newsletter-drafter, community-content-drafter, etc.) asking to refine a draft to a quality bar. Also trigger aggressively inside any house content drafting flow that has a matching rubric in quality-judge/rubrics/.
---
...
2. **Preserve voice.** The profile rules (brand-voice.md, the client house rules) still apply. Judge feedback is ADDITIVE to the voice floor, never a license to violate it.
