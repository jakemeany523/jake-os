...
Generates branded mp4 videos from live evaluation data. Videos are React components where time is a prop. Data goes in as JSON, branded mp4s come out. Integrated into the Social Content Drafter so data-heavy posts auto-trigger video rendering alongside static graphics.

## Architecture

```
Platform SDK  -->  Python glue script  -->  JSON props  -->  Remotion render  -->  mp4
                                                                                  |
                                                                           Buffer / social
```
...
|----------|-----------|----------|----------|
| eval-drop | 1920x1080 | 6s | Animated leaderboard reveal, top N models with score bars |
| eval-drop-vertical | 1080x1920 | 6s | Same, vertical for Reels/Shorts |
...
- the company logo bottom-right on every frame
- Footer: "Evaluated on the platform by our team"
- No dates, no pricing

## Integration Points

- **Platform SDK:** Live evaluation data for template props
