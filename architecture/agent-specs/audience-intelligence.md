...
The output feeds directly into Clay (enrichment) and the CRM (pipeline), creating a direct line from a Twitter post to a named contact in the sales pipeline.
...
```bash
export MIXPANEL_API_KEY="${MIXPANEL_API_KEY}"
...
4. Identify the top 20-30 accounts that replied, quoted, or mentioned the company account this week
...
|---------------|--------|
| >= 13 | Push to the CRM as lead. Flag for the sales team. |
...
| the CRM leads (score >= 13) | the CRM | the sales pipeline view |
...
1. **Today:** Mixpanel behavior data + Clay enrichment + the CRM routing. Working.
2. **After an engineering fix (this week):** UTM data populates. Can trace specific posts to site visits.
3. **After an engineering fix (this week):** External referrer populates. Can see Twitter vs. LinkedIn vs. Google traffic.
